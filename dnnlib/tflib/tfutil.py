...
"""Miscellaneous helper utils for TensorFlow 2.x."""
...
def shape_to_list(shape: Iterable[tf.Dimension]) -> List[Union[int, None]]:
    """Convert a TensorFlow shape to a list of ints. Retained for backward compatibility."""
    return [dim if dim is None else dim.value for dim in shape]
...
def erfinv(y: TfExpressionEx) -> TfExpression:
    ...
    from tensorflow.python.ops.special_math_ops import erfinv
    return erfinv(y)
...
def assert_tf_initialized():
    ...
    if tf.compat.v1.get_default_session() is None:
        raise RuntimeError("No default TensorFlow session found. Please call init_tf().")
